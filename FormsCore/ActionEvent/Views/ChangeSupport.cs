...
public class ChangeSupport: MyValidatableObject
{
    ...
    /// </summary>
    public bool EnableVerticalBinding = false;
    public bool EnableChangeSupport = false;


    

    public virtual void SendEvent(object messageEvent) => BeforeSendEvent(messageEvent);


    public virtual void BeforeSendEvent(object messageEvent) => Writing.ToConsole(messageEvent.GetType().Name);



    protected void FireEvent(PropertyChangedMessage  propertyChangedMessage) => Writing.ToConsole(propertyChangedMessage.GetType().Name);
    protected void FireEvent(CommonEventMessage<string> message)=> Writing.ToConsole(message.GetType().Name);
